...
`;
  ...
  const DLsite = {
    parseWorkData: function (json, rj) {
      ...
      workInfo.cv = json[0].creaters.voice_by
        ? json[0].creaters.voice_by.map((r) => {
            return r.name;
          })
        : "无CV";
    ...
    request: function (rjCode, callback) {
      const url = `https://www.dlsite.com/home/api/=/product.json?workno=${rjCode}`;
      getXmlHttpRequest()({
        method: "GET",
        url,
        headers: {
